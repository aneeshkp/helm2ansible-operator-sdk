...
REPO = github.com/redhat-nfvpe/helm2ansible-operator-sdk
BUILD_PATH = $(REPO)
...
all: format test build/helm2ansible-operator-sdk
...
ci-build: build/helm2ansible-operator-sdk-$(VERSION)-x86_64-linux-gnu ci-install

ci-install:
	mv build/helm2ansible-operator-sdk-$(VERSION)-x86_64-linux-gnu build/helm2ansible-operator-sdk

release_x86_64 := \
	build/helm2ansible-operator-sdk-$(VERSION)-x86_64-linux-gnu \
	build/helm2ansible-operator-sdk-$(VERSION)-x86_64-apple-darwin

release: clean $(release_x86_64) $(release_x86_64:=.asc)

build/helm2ansible-operator-sdk-%-x86_64-linux-gnu: GOARGS = GOOS=linux GOARCH=amd64
build/helm2ansible-operator-sdk-%-x86_64-apple-darwin: GOARGS = GOOS=darwin GOARCH=amd64
